...
### Labs
For this chapter you will need to set up an NFS server preferably on a different VM.

1. `dnf install nfs-utils` - Install the NFS server
1. `systemctl enable --now nfs-server` - Start the server automatically
1. `mkdir -p /shares/public` - Create the root share directory and the directory used for section 2 lab
1. Create `/etc/exports` with the following lines.  Replace `10.0.0.0` with your local subnet value.
````
/shares		   10.0.0.0/24(rw,sync,no_subtree_check,fsid=0)
/shares/public	10.0.0.0/24(rw,sync,no_subtree_check)
````
1. `exportfs -ra` - Update the NFS configuration with the new exports
1. `exportfs -v` - Verify the configuration.  You should see the new directories listed
